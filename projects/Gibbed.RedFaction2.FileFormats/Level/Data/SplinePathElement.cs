...
{
    public class SplinePathElement : ISerializableElement
    {
        ...
        public void Read(Stream input, uint version, Endian endian)
        {
            ...
            var unknown6 = version >= 256 ? input.ReadValueB8() : false;
            var unknown7 = version >= 257 ? input.ReadValueS32(endian) : -1;
            
            var unknown8 = input.ReadValueU32(endian);
            for (uint i = 0; i < unknown8; i++)
            {
                var unknown9 = input.ReadValueU32(endian);
                var unknown10 = Vector3.Read(input, endian);
                var unknown11 = Vector3.Read(input, endian);
                var unknown12 = Vector3.Read(input, endian);
                var unknown13 = input.ReadValueU32(endian);
                var unknown14 = Vector4.Read(input, endian);
                var unknown15 = input.ReadValueF32(endian); // value * 127.0f
                var unknown16 = input.ReadValueF32(endian); // value * 127.0f
                var unknown17 = input.ReadValueF32(endian); // value * 127.0f
                var unknown18 = input.ReadValueF32(endian); // value * 2.54f
                var unknown19 = input.ReadValueF32(endian); // value * 2.54f
            }

            var unknown20 = version >= 238 ? input.ReadValueS32(endian) : -1;
        }
